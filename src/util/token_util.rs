...
impl TokenUtil {
    ...
    // token验证 (自带超时验证)
    pub fn decode_token(token: String) -> jsonwebtoken::errors::Result<TokenData<TokenUtil>> {
        jsonwebtoken::decode::<TokenUtil>(&token, &DecodingKey::from_secret(&KEY),  &Validation::new(ALGORITHM))
    }

    // 验证token (目前仅验证用户名)
    pub fn verify_token(token_data: &TokenData<TokenUtil>) -> Result<(),()> {
    ...
    pub fn get_username(req:HttpRequest)->Result<String,String>{
        ...
        if let Some(auth_header) = auth_token{
            ...
            if auth_str.starts_with("bearer") || auth_str.starts_with("Bearer") {
                ...
                if let Ok(token_data) = TokenUtil::decode_token(token.to_string()) {
                    if TokenUtil::verify_token(&token_data).is_ok() {
